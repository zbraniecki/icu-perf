        ...
        <Self::Locale as FromStr>::Err: Debug,
    {
        ...
        let parsed = {
            let test = Test {
                ...
                description: format!(
                    "Create LanguageIdentifier from str for {} locales",
                    inputs.len()
                ),
                unit: "ns".into(),
            };
            ...
            let output: Vec<Self::Locale> = inputs.iter().map(|s| s.parse().unwrap()).collect();
        ...
        suit_result.print().expect("Printing succeeded");
    }
}
